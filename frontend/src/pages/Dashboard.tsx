...
const DashboardSkeleton = () => (
  <div className="p-6 md:p-8 space-y-6 animate-pulse">
    <div className="h-32 bg-neutral-800/60 rounded-2xl" />
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="h-28 bg-neutral-800/60 rounded-2xl" />
      ))}
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 h-80 bg-neutral-800/60 rounded-2xl" />
      <div className="h-80 bg-neutral-800/60 rounded-2xl" />
    </div>
    <div className="h-96 bg-neutral-800/60 rounded-2xl" />
  </div>
);
...
export const Dashboard = () => {
  ...
  // Memoize filtered predictions to avoid re‑filtering on every render
  const filteredPredictions = useMemo(() => {
    if (!data?.predictions) return [];
    const query = searchQuery.toLowerCase().trim();
    if (!query) return data.predictions;
  ...
  if (isError || !data) {
    return (
      <div className="p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-white">Failed to load telemetry</h2>
  ...
  const { health, metrics, distribution, trends } = data;
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          title="Total Packet Scans"
          value={metrics.totalInspections.toLocaleString()}
          icon={Activity}
          trend={metrics.inspectionDeltaPct}
          trendLabel="+ from last hour"
          subtitle="High volume network interface eth0"
          color="indigo"
        />
        <KPICard
          ...
          icon={ShieldAlert}
          trend={metrics.threatDeltaPct}
          trendLabel="vs previous period"
          subtitle="Blocked via automated firewall rules"
          color="red"
        />
        <KPICard
          ...
          icon={CheckCircle2}
          subtitle="Evaluated on test payload validation set"
          color="emerald"
        />
        <KPICard
          ...
          icon={Zap}
          subtitle="Sub-20ms real‑time SLA maintained"
          color="amber"
        />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-neutral-800/60 text-neutral-300">
              {filteredPredictions.map((row: PredictionRecord) => (
